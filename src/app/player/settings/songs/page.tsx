...
const SettingsSongs = () => {
  ...
  useEffect(() => {
    ...
    if(configData) {
      setFormData({
        songDirectory: configData.song_directory,
      })
    }
  ...
  const validateForm = () => {
    ...
    if (!formData.songDirectory) {
      newErrors.songDirectory = "Song directory is required";
      isValid = false;
    } else {
      newErrors.songDirectory = "";
    }
  ...
  const handleSubmit = () => {
    ...
    if (validateForm()) {
      ...
      var newData = {
        "songDirectory" : formData.songDirectory,
      };
      ...
      axios
          ...
          .then(response => {
              ...
              if (status == 1) {
                loadConfig();
                toast.update(id_toast, {render: "Song directory updated", type: "success", isLoading: false, autoClose: Number(process.env.NEXT_PUBLIC_TIMEOUT_TOAST) });
              }
          })
          .catch(function (error) {
              const message = error.response.data.message ? error.response.data.message : "Something went wrong";
  ...
  const handleScan = () => {
    ...
    axios
        .get(process.env.NEXT_PUBLIC_API_URL + "/songs/scan")
        .then(response => {
            var data = response.data;
            var status = data.status;
            if (status == 1) {
              var count = data.songs_count;
              toast.update(id_toast, {render: count + " songs were recorded", type: "success", isLoading: false, autoClose: Number(process.env.NEXT_PUBLIC_TIMEOUT_TOAST) });
            }
        })
        .catch(function (error) {
            const message = error.response.data.message ? error.response.data.message : "Something went wrong";
            ...
            <div className="py-6 rounded-lg">
              ...
              >
                  <div id="form-scans" className="flex flex-col justify-between space-y-2 rounded-lg bg-black p-3.5 lg:px-5 lg:py-3 ">
                    ...
                    <div>
                      <div className="mb-4">
                        <label className="block text-white-700 font-medium mb-2" htmlFor="songDirectory">
                          Songs
                        </label>
                        <input
                          ...
                          value={formData.songDirectory}
                        />
                        {errors.songDirectory && <p className="text-red-500 italic">{errors.songDirectory}</p>}
                      </div>
                      ...
                      <div className="flex flex-col items-center py-10 px-10 mx-0 min-w-full">
                        <div className="inline-flex">
                          ...
                          <button
                            onClick={() =>  router.push("/player/songs")}
                            className="ml-2 btn-lg-form"
                          >
                            Return
                          </button>
...
export default useAuth(SettingsSongs);