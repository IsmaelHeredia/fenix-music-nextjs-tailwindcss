...
const CreateStation = () => {
  ...
  const handleSubmit = () => {
    ...
    if (validateForm()) {
      ...
      axios
          .post(process.env.NEXT_PUBLIC_API_URL + "/stations", newData)
          .then(response => {
              var data = response.data;
              var status = data.status;
              if (status == 1) {
                toast.update(id_toast, {render: "Station created", type: "success", isLoading: false, autoClose: Number(process.env.NEXT_PUBLIC_TIMEOUT_TOAST)});
...
export default useAuth(CreateStation);